...
describe('Students API (e2e)', () => {
  ...
  describe('Autres opérations', () => {
    ...
    it('6. POST avec données valides doit renvoyer 201 + ID', async () => {
      const res = await request(app.getHttpServer()).post('/students').send(validStudent).expect(201);
      expect(res.body.id).toBe(6);
    });

    it('7. POST sans champ obligatoire doit renvoyer 400', () => {
      return request(app.getHttpServer()).post('/students').send({ firstName: 'Test' }).expect(400);
    });

    it('8. POST avec note invalide (25) doit renvoyer 400', () => {
      return request(app.getHttpServer()).post('/students').send({ ...validStudent, grade: 25 }).expect(400);
    });

    it('9. POST avec email déjà existant doit renvoyer 409', () => {
      return request(app.getHttpServer()).post('/students').send({ ...validStudent, email: 'jean@test.fr' }).expect(409);
    });

    it('10. PUT avec données valides doit renvoyer 200', async () => {
      const res = await request(app.getHttpServer()).put('/students/1').send(validStudent).expect(200);
      expect(res.body.firstName).toBe('Kyllian');
    });

    it('11. PUT avec ID inexistant doit renvoyer 404', () => {
      return request(app.getHttpServer()).put('/students/999').send(validStudent).expect(404);
    });
    ...
    it('14. GET /students/stats doit renvoyer les propriétés obligatoires', async () => {
      const res = await request(app.getHttpServer()).get('/students/stats').expect(200);
    ...
    it('15. GET /students/search?q=marie doit renvoyer Marie Curie', async () => {
      const res = await request(app.getHttpServer()).get('/students/search?q=marie').expect(200);
